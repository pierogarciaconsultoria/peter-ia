...
export function JobSalaryPlan() {
  ...
  const handleApprove = (jobPosition: JobPosition) => {
    const updatedPositions = jobPositions.map(pos => pos.id === jobPosition.id ? {
      ...pos,
  ...
  const handleSaveJobPosition = (updatedJobPosition: JobPosition) => {
    if (selectedJobPosition) {
      ...
      setJobPositions(jobPositions.map(pos => pos.id === updatedJobPosition.id ? updatedJobPosition : pos));
      toast({
  ...
  return <div className="space-y-6">
      ...
      <Tabs defaultValue="positions">
        ...
        <TabsContent value="positions">
          <JobPositionTable jobPositions={jobPositions} onViewDetails={handleViewDetails} onApprove={handleApprove} onRevise={handleRevise} onDistribute={handleDistribute} />
        </TabsContent>
        ...
        <TabsContent value="orgchart">
          <div className="space-y-4">
            ...
            <DepartmentOrgChart positions={jobPositions.map(job => ({
            id: job.id,
            title: job.title,
      ...
      <JobPositionDialog isOpen={isNewJobDialogOpen} onClose={() => setIsNewJobDialogOpen(false)} onSave={handleSaveJobPosition} />
      
      {/* Dialog for editing existing job positions */}
      {selectedJobPosition && <JobPositionDialog isOpen={isEditDialogOpen} onClose={() => setIsEditDialogOpen(false)} onSave={handleSaveJobPosition} jobPosition={selectedJobPosition} />}
      
      {/* Dialog for viewing job position details */}
      {selectedJobPosition && <JobPositionDetailsDialog isOpen={isDetailsDialogOpen} onClose={() => setIsDetailsDialogOpen(false)} jobPosition={selectedJobPosition} onEdit={handleEditFromDetails} onApprove={() => {
